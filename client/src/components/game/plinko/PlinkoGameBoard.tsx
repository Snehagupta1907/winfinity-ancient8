...
import { LinesType, MultiplierValues } from '@/types/types';
import {
    getMultiplierByLinesQnt,
    getMultiplierSound
  } from './config/multipliers'
import { config } from './config'
const PlinkoGameBoard: React.FC = () => {
  
    const engine = Engine.create()
    const [lines, setLines] = useState<LinesType>(16)
    const inGameBallsCount = 5;
    const incrementInGameBallsCount = 1;
    const decrementInGameBallsCount = 1;
    ...
    return (
      <div className="flex h-fit flex-col-reverse items-center justify-center gap-4">
        <BetActions
